...
/* Banka hesabini temsil eden yapi */
typedef struct {
    int accountNumber;           /* Benzersiz hesap numarasi */
    char name[MAX_NAME_LENGTH];  /* Hesap sahibinin adi */
    double balance;              /* Guncel hesap bakiyesi */
} Account;
...
/* Girdi hatalarini onlemek icin girdi tamponunu temizler */
void clearInputBuffer() {
    int c;
    while ((c = getchar()) != '\n' && c != EOF);
}

/* Benzersiz hesap numarasi uretir */
int generateAccountNumber() {
    if (accountCount == 0) {
        return 1001;
    }
...
/* Hesap numarasina gore hesap arar */
int findAccount(int accountNumber) {
...
/* T├╝m hesaplar─▒ dosyadan belle─şe y├╝kler */
void loadAccounts() {
    FILE *file = fopen(DATA_FILE, "rb");
    if (file == NULL) {
        /* Dosya henuz mevcut degil, bos hesap listesiyle basla */
        accountCount = 0;
        return;
    }
